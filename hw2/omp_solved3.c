...
float a[N], b[N], c[N], d[N];   // create independent variable to make paralleize
...
#pragma omp parallel private(c,i,tid,section) num_threads(2) 
  ...
  #pragma omp sections nowait
    ...
    #pragma omp section
      ...
      section = 2;
      for (i=0; i<N; i++)
        d[i] = a[i] + b[i];         //use d[N] to let variable independent from section 1
      print_results(d, tid, section);
      }
